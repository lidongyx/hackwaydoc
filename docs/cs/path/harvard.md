...
## 推荐学习课程
### ① CS32 计算思维和问题解决
介绍计算思维、计算机科学领域的有用概念以及使用 Python 进行计算机编程的艺术。重点放在课堂会议和学习使用计算机解决复杂的现实问题上。引入概念和技术，因为它们需要帮助解决我们面临的问题。学生将学习如何从一个模棱两可的问题描述转变为一个运行的解决方案，并将让课堂知道如何指导计算机做他们想让他们做的事情。不需要计算机科学或计算机编程方面的先前经验。
...
### ④ CS109 数据科学概论
数据科学 1 是为期一年的数据科学导论的前半部分。该课程将侧重于分析杂乱的现实生活数据，以使用统计和机器学习方法进行预测。涵盖的材料将使用数据整合调查的五个关键方面：（1）数据收集 - 数据整理、清理和采样以获得合适的数据集；(2) 数据管理——快速可靠地访问数据；(3) 探索性数据分析——产生假设和建立直觉；(4) 预测或统计学习；(5) 交流——通过可视化、故事和可解释的摘要来总结结果。两部分系列的第一部分。本课程的课程贯穿整个学年。
